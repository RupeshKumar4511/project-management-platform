...
export const updateUser = createAsyncThunk('auth/update-user', async (userData,thunkAPI) => {
    try {
        await ensureAuth();
        const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/v1/users/update`, {
            method: "PATCH",
            headers: { "Content-Type": 'application/json' },
            credentials: "include",
            body: JSON.stringify(userData)
...
export const sendMail = createAsyncThunk('auth/sendMail', async (userData,thunkAPI) => {
    try {
        const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/v1/auth/send-email`, {
            method: "POST",
            headers: { "Content-Type": 'application/json' },
            body: JSON.stringify(userData)
        })
...
export const verifyUser = createAsyncThunk('auth/verifyUser', async (userData,thunkAPI) => {
    try {
        const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/v1/auth/verify-user`, {
            method: "POST",
            headers: { "Content-Type": 'application/json' },
            body: JSON.stringify(userData)
        })
