...
export const sendMail = createAsyncThunk('auth/sendMail', async (userData,thunkAPI) => {
    try {
        const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/auth/send-email`, {
            method: "POST",
            headers: { "Content-Type": 'application/json' },
            body: JSON.stringify(userData)
        })
...
export const verifyUser = createAsyncThunk('auth/verifyUser', async (userData,thunkAPI) => {
    try {
        const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/auth/verify-user`, {
            method: "POST",
            headers: { "Content-Type": 'application/json' },
            body: JSON.stringify(userData)
        })
